...
- **代码架构优化**：
  ...
  - 提高了代码复用性和维护性，确保功能在主界面、历史数据查看、自动保存等所有场景中的一致性
...
6. 在侧边栏中可以调整PRPD图的类型、累积周期数、参考正弦波、颜色方案等
7. 使用"单位"按钮可以在dBm和mV单位之间切换，轴范围和正弦波参数会自动调整
8. 在侧边栏中可以控制数据是否保存到数据库、查看数据库、导出CSV、自动保存图像等
9. 使用"查看文件路径"按钮可以查看数据库文件和图像保存的具体位置
